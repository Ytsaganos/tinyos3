...
rlnode SCHED[MAX_LEVEL]; /* The scheduler queue */
...
void yield(enum SCHED_CAUSE cause)
{
	...
	case SCHED_IO:
		if(cur_thread() -> priority < MAX_LEVEL -1)
			cur_thread() -> priority++;
		break;

	case SCHED_MUTEX:
		if(((cur_thread()-> curr_cause) == (cur_thread() -> last_cause && (cur_thread() -> curr_cause) == SCHED_MUTEX)))
		{
			if(cur_thread() -> priority > 0)
				cur_thread() -> priority--;
		}
